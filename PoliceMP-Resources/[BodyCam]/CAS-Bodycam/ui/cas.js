...
const DisplayBodycam = function(show, name, grade, header, desc, webhook) {
    ...
    const sound = document.getElementById("bodycamStartSound")
    if (sound) {
        sound.volume = 0.1
    ...
    $("div.bodycam-main").fadeIn(500, function() {
        ...
        $("#jobGrade").html(grade)
        StartTimeEvent()
        StartRecording(name, desc, webhook)
...
function StartRecording(name, desc, webhook) {
  // Wait if MainRender is not initialized yet
  if (typeof MainRender === "undefined" || typeof MainRender.createTempCanvas !== "function") {
    console.warn("MainRender not ready, retrying...");
    setTimeout(() => StartRecording(name, desc, webhook), 250);
    return;
  }
  ...
  mediaRecorder.onstop = function () {
    ...
    formData.append("payload_json", JSON.stringify({
      embeds: [
        {
          title: `🎥 Bodycam Footage Uploaded by **${name}**`,
          color: 0x26C7FA,
    ...
    .then(data => {
      if (data.attachments && data.attachments.length > 0) {
        ...
        $.post(gamePost + "getVideoURL", JSON.stringify({
          ...
          videoName: name,
          videoDesc: desc,
          recorder: name // 👈 send it to server as "recorder"
        }), function (result) {
          if (result === "ok") {
...
window.addEventListener("keydown", (event) => {
  ...
  $(document).ready(function() {
    ...
    $.post(gamePost + "escapeFromNUI", JSON.stringify({}))
    })
