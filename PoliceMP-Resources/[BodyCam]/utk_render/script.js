...
class GameRender {
    constructor() {
        ...
        this.addOverlay("nui://utk_render/overlay1.png");
    }

    async addOverlay(url) {
        this.overlayUrl = url;

        const texture = await loadOverlayTexture(url);
        ...
        const overlayPlane = new PlaneBufferGeometry(window.innerWidth, window.innerHeight);
        this.overlayQuad = new Mesh(overlayPlane, overlayMaterial);
        this.overlayQuad.position.z = -99;

        this.sceneRTT.add(this.overlayQuad);
    }

    resize(screenshot = false) {
        const cameraRTT = new OrthographicCamera(
            window.innerWidth / -2,
            window.innerWidth / 2,
            window.innerHeight / 2,
        ...
        if (this.overlayQuad) {
            sceneRTT.add(this.overlayQuad); // Re-add overlay after resizing
        }
    ...
    renderToTarget(element) {
        this.resize(true); // Always full resolution
        this.canvas = element;
        isAnimated = true;
    }
