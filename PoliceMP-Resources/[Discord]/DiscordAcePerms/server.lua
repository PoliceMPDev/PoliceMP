...
AddEventHandler('playerDropped', function (reason) 
	...
	if PermTracker[discord] ~= nil then 
		...
		local rolesRemoved = 0
		for i = 1, #list do 
			local permGroup = list[i];
			ExecuteCommand('remove_principal identifier.discord:' .. discord .. " " .. permGroup);
			rolesRemoved = rolesRemoved + 1
		end		
		local playerName = GetPlayerName(src) or "Unknown"
        print("[DiscordAcePerms] (playerDropped) Removed " .. playerName .. " from " .. rolesRemoved .. " role groups.")
...
AddEventHandler('playerConnecting', function(name, setKickReason, deferrals)
    ...
    -- Check if discord identifier is not nil or empty
    if discord == nil or discord == "" then
        print("[DiscordAcePerms] Discord identifier not found for player " .. (GetPlayerName(src) or "Unknown"))
        deferrals.done("Discord identifier not found. Please link your Discord.")
        return
    end
    ...
    if identifierDiscord then
        ...
        local roleIDs = exports.DiscordAPI:GetDiscordRoles(src)
        if debugScript then 
        ...
        if not (roleIDs == false) then
            ...
            for i = 1, #roleList do
                for j = 1, #roleIDs do
                    if exports.DiscordAPI:CheckEqual(roleList[i][1], roleIDs[j]) then
            ...
            print("[DiscordAcePerms] (playerConnecting) Added " .. (GetPlayerName(src) or "Unknown") .. " to " .. rolesAdded .. " role groups.")
        else
            print("[DiscordAcePerms] " .. (GetPlayerName(src) or "Unknown") .. " has not gotten permissions because we could not find their roles...")
        ...
        if DiscordDetector[license] == nil then 
            ...
            print('[DiscordAcePerms] Discord was not found for player ' .. (GetPlayerName(src) or "Unknown") .. "...")
            local clicked = false;
            while not clicked do 
                deferrals.presentCard(card,
                function(data, rawData)
