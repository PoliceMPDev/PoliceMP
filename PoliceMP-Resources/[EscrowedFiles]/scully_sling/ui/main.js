const app = Vue.createApp({
    ...
    computed: {
        totalPages() {
            return Math.ceil(this.weapons.length / this.itemsPerPage);
        },
        displayedWeapons() {
            const startIndex = (this.selectedPage - 1) * this.itemsPerPage;
    ...
    methods: {
        fetchNui(eventName, data) {
            fetch(`https://scully_sling/${eventName}`, {
                method: 'post',
                headers: {
        ...
        toggleOption(weapon, option) {
            if (option === 'custom') {
                this.editingWeapon = weapon.custom;
                this.editingWeapon.hash = weapon.hash;
                this.displayEdit = true;
            } else {
                weapon.selected = option;

                this.fetchNui('updateWeapon', {weapon: weapon})
            };
        },
        resetEdit() {
            const index = this.weapons.findIndex(weapon => weapon.hash === this.editingWeapon.hash);

            if (index !== -1) {
                const weapon = this.weapons[index];

                this.fetchNui('updateWeapon', {hash: weapon.hash})
            }
        ...
        saveEdit() {
            if (!this.editingWeapon.bone) {
                this.fetchNui('notify', {text: this.language.requireBone})
                return;
            }
        
            const posOffsetArray = this.editingWeapon.offset ? this.editingWeapon.offset.split(',').map(coord => parseFloat(coord.trim())) : [];

            if (posOffsetArray.length !== 3 || posOffsetArray.some(isNaN)) {
                this.fetchNui('notify', {text: this.language.requireOffset})
                return;
            }

            const rotationArray = this.editingWeapon.rotation ? this.editingWeapon.rotation.split(',').map(coord => parseFloat(coord.trim())) : [];

            if (rotationArray.length !== 3 || rotationArray.some(isNaN)) {
                this.fetchNui('notify', {text: this.language.requireOffset})
                return;
            }
            ...
            if (index !== -1) {
                ...
                weapon.selected = 'custom';
                weapon.custom = {
                    bone: this.editingWeapon.bone,
                    offset: this.editingWeapon.offset,
                    rotation: this.editingWeapon.rotation
                };
