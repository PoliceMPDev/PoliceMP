local Radio = {
	...
	Enabled = true,
	Handle = nil,
	Prop = GetHashKey('prop_cs_walkie_talkie'), -- only ran once and doesn't break my syntax viewer
	Bone = 28422,
...
Radio.Labels = {
	{ "FRZL_RADIO_HELP", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " to hide."},
	{ "FRZL_RADIO_HELP2", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " to hide."},
}

AddEventHandler('policemp-radio:power-on', function()
    Radio.On = true
...
AddEventHandler('policemp-radio:power-off', function()
    Radio.On = false
...
AddEventHandler('policemp-radio:change-channel', function(channel)
    Radio:Remove(radioConfig.Frequency.Current)
...
AddEventHandler('policemp-radio:stop-modification', function()
    Radio:Toggle(false)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		-- Init local vars
		local playerPed = PlayerPedId()
		local isActivatorPressed = IsControlJustPressed(1, radioConfig.Controls.Activator.Key)
		local isSecondaryPressed = (radioConfig.Controls.Secondary.Enabled == false and true or IsControlPressed(1, radioConfig.Controls.Secondary.Key))
		local isFalling = IsPedFalling(playerPed)
		local isDead = IsEntityDead(playerPed)
		local minFrequency = radioConfig.Frequency.List[1]
		...
		if isActivatorPressed and isSecondaryPressed and not isFalling and Radio.Enabled and Radio.Has and not isDead then
            ...
            TriggerEvent('rp-radio:police-mp-toggle-modification', Radio.Open)
		elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
			Radio:Remove()
			exports["pma-voice"]:setVoiceProperty("radioEnabled", false)
			Radio:Toggle(false)
