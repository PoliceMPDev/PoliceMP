...
const postStatus = (status) => {
  $.post(`http://${GetParentResourceName()}/zCustody-Alarm:postStatus`, JSON.stringify(status));
};
...
const playAudioLoop = (soundName) => {
  ...
  if (audio) {
    ...
    setTimeout(() => playAudioLoop(soundName), 900);
  }
};
...
const formatDate = () => {
  ...
  return `${address} : ${dayName} ${day} ${monthName} ${year}`;
};
...
const activateButtonClickHandler = () => {
  updatePanelUI(true, 'ALARM ACTIVATION: Control Panel');
  $.post(`http://${GetParentResourceName()}/zCustody-Alarm:activateButton`, JSON.stringify(systemId));
};

const resetButtonClickHandler = () => {
  updatePanelUI(false, '');
  $.post(`http://${GetParentResourceName()}/zCustody-Alarm:resetButton`, JSON.stringify(systemId));
};
