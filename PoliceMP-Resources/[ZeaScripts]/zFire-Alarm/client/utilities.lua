...
util.notification = function(string, type)
    ...
    -- SetNotificationMessage('CHAR_WE', 'logo', true, 4, 'zFire-Alarm', 'By Zea Development')
...
util.notifyUponActivation = function(panelId, buildingName, buildingAddress)
    if ( not cfg.createCad ) then 
        -- if ( fireDuty ) then 
            ...
            TriggerServerEvent('chat:addMessage', {
                color = { 255, 0, 0},
                multiline = true,
                args = {'Emergency Control Room', '\nNew Incident: Alarm Activation \n Location: ' .. buildingName .. ' : ' .. buildingAddress}
            })
        -- end
    else
        TriggerServerEvent("zFire-Alarm:createCad", { 
            type = 'Fire - Alarm Activation',
            description = 'Alarm Activation reported at ' .. buildingName,
            location = buildingAddress
        })
    end
end

-- @param: Initiate a fire presence check.
-- @param: This function was developed to enable additional customization of the resource, allowing you to integrate support for your own fire scripts.
util.detectionType = function(coords)
    if ( cfg.fireDetection.type == 'baseGame' ) then 
        boolean, pos = GetClosestFirePos(coords)
    end

    if ( boolean and pos ~= nil ) then 
        return boolean, pos
    end
end
