-- Edited for fms name compatibility
...
local function routeMessage(source, author, message, mode, fromConsole)
    ...
    if source >= 1 then
		...
		exports["fms"]:getPlayerFmsData(source, function(playerdata)
			if playerdata ~= nil then
				author = playerdata.shortdisplayname
			end		
			process()
		end)
    else
...
AddEventHandler('__cfx_internal:commandFallback', function(command)
	...
	exports["fms"]:getPlayerFmsData(source, function(playerdata)
		if playerdata ~= nil then
			name = playerdata.shortdisplayname
		end		
...
AddEventHandler('playerJoining', function()
	...
	exports["fms"]:getPlayerFmsData(source, function(playerdata)
		...
		TriggerClientEvent('chatMessage', -1, '', { 255, 255, 255 }, '^2* ' .. name .. ' joined.')
	end)
...
AddEventHandler('playerDropped', function(reason)
	...
	exports["fms"]:getPlayerFmsData(source, function(playerdata)
		...
		TriggerClientEvent('chatMessage', -1, '', { 255, 255, 255 }, '^2* ' .. name ..' left (' .. reason .. ')')
	end)
end)
