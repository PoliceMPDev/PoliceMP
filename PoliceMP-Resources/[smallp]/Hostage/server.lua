...
RegisterCommand("+xth", function(source)
    if IsPlayerAceAllowed(source, "group.TierTwo") then
        TriggerClientEvent("Hostage:TakeHostage", source)
    end
end, false)
...
AddEventHandler("TakeHostage:stop", function(targetSrc)
	...
	if takingHostage[source] then
		TriggerClientEvent("TakeHostage:pmp_stop", targetSrc)
		takingHostage[source] = nil
		takenHostage[targetSrc] = nil
...
AddEventHandler('playerDropped', function(reason)
	...
	if takingHostage[source] then
		TriggerClientEvent("TakeHostage:pmp_stop", takingHostage[source])
		takenHostage[takingHostage[source]] = nil
		takingHostage[source] = nil
	end

	if takenHostage[source] then
		TriggerClientEvent("TakeHostage:pmp_stop", takenHostage[source])
