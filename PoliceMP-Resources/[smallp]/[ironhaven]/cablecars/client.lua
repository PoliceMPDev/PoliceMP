...
Citizen.CreateThread(function()
	...
	if CABLE_CARS[0].showTramBlips then
		...
		AddTextComponentString("PoliceMP Sky Link")
...
RegisterNetEvent("pmp:cablecar:forceState")
AddEventHandler("pmp:cablecar:forceState", function(index, state)
...
function DrawSimpleText(text, x, y, scale, font, r, g, b, a)
    -- Default values
    scale = scale or 0.35
    font = font or 4
    r = r or 255
    ...
    SetTextOutline()
    SetTextCentre(1) -- Centers the text

    -- Add the text and draw it
    SetTextEntry("STRING")
    ...
    elseif cablecar.state == "MOVE_DOWN" then
        ...
        if cablecar.run_timer > 1.0 then
            ...
            if cablecar.gradient <= 1 then
                ...
                cablecar.state = "IDLE"
                cablecar.gradient_distance = 0.0
                TriggerServerEvent("pmp:cablecar:host:sync", cablecar.index, "IDLE_BOTTOM")
                return
            end
...
function GivePlayerOptionToJoinMyCablecar(cablecar, moving)
    ...
    if not cablecar.is_player_seated then
        ...
        local dist = #(pos - plypos)
        if dist < 2.0 then
            DrawSimpleText("Press ~g~[E] ~w~to enter", 0.5, 0.95, 0.4, 4, 255, 255, 255, 255)
        ...
        if not moving then
            DrawSimpleText("Press ~g~[E] ~w~to exit", 0.5, 0.95, 0.4, 4, 255, 255, 255, 255)
            if IsControlJustPressed(0, 38) then
                cablecar.is_player_seated = false
                DetachEntity(ply, 0, 0)
            end
