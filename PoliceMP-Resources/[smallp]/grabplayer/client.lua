...
function L20_1()
  ...
  if nil == L0_2 then
    ...
    L0_2 = L12_1
    if L0_2 then
    local pedPlayer1 = L17_1(pedInFront)
    local pedPlayer2 = L17_1(L0_1)

    if pedPlayer1 ~= -1 and pedPlayer2 ~= -1 then
        local serverId1 = GetPlayerServerId(pedPlayer1)
        local serverId2 = GetPlayerServerId(pedPlayer2)

        if serverId1 and serverId2 then
            local netPed1 = PedToNet(pedInFront)
            local netPed2 = PedToNet(L0_1)
            TriggerServerEvent("pmp:sendEvent", serverId1, serverId2, netPed1, netPed2)
        else
            print("[pmp] Failed to get server IDs.")
        end
    else
        print("[pmp] One of the players is not valid.")
    end
