...
function GetFuel(vehicle)
    print(GetHashKey("bzzz_pumps_charger_a"))
...
function nearPump(coords)
    local entity = nil
    for hash in pairs(config.elecPump) do
    ...
    if config.elecPump[GetEntityModel(entity)] then
...
function vehicleInFront()
    ...
    if IsEntityAVehicle(entity) then
        ...
        if config.electricVehicles[vehModel] then
            ...
            DrawText3D(vehCoords.x, vehCoords.y, vehCoords.z + 1.2, "~r~This is not an electric vehicle.")
...
function grabNozzleFromPump()
    ...
    nozzle = CreateObject(`prop_charger`, 0, 0, 0, true, true, true)
    AttachEntityToEntity(nozzle, ped, GetPedBoneIndex(ped, 0x49D9), 0.11, 0.02, 0.02, -90.0, 180.0, 0.0, true, true, false, true, 1, true)
    RopeLoadTextures()
    ...
    rope = AddRope(pump.x, pump.y, pump.z, 0.0, 0.0, 0.0, 3.0, 3, 1000.0, 0.0, 1.0, false, false, false, 1.0, true)
    while not rope do
        Wait(0)
    end
    ...
    nozzlePos = GetOffsetFromEntityInWorldCoords(nozzle, 0.05, -0.010, -0.075)
...
function putNozzleInVehicle(vehicle, ptankBone, isBike, dontClear, newTankPosition)
    TriggerServerEvent("eforcePassengerExit", NetworkGetNetworkIdFromEntity(vehicle))
    local vehModel = GetEntityModel(vehicle)

    if not config.electricVehicles[vehModel] then
        DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z + 1.0, "~r~This vehicle cannot be charged.")
        return
    end
    ...
    if isBike then
        ...
        AttachEntityToEntity(nozzle, vehicle, ptankBone, -0.17 + newTankPosition.x, -0.20 + newTankPosition.y, 0.48 + newTankPosition.z, 180.0, 180.0, 0.0, true, true, false, false, 1, true)
    end
...
local function vehicleIsFueling()
    if not vehicleFueling then return end

    local vehModel = GetEntityModel(vehicleFueling)
    if not config.electricVehicles[vehModel] then
        DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z + 1.0, "~r~This vehicle cannot be charged.")
        vehicleFueling = false
        return
...
CreateThread(function()
    while true do
        ...
        if vehicleFueling then
            ...
            DrawText3D(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 1.2, string.format("Charging: %.1f%%", GetFuel(vehicleFueling)))
        end
    end
end)
    ...
    while true do
        ...
        if pump then
            ...
            if not holdingNozzle and not nozzleInVehicle and not nozzleDropped then
                DrawText3D(pump.x, pump.y, pump.z + 1.2, "[E] Grab Charger")
            ...
            elseif holdingNozzle and not nearTank and pumpHandle == usedPump then
                DrawText3D(pump.x, pump.y, pump.z + 1.2, "[E] Return Charger")
                if IsControlJustPressed(0, 51) then
                    LoadAnimDict("anim@am_hold_up@male")
                    TaskPlayAnim(ped, "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                    Wait(300)
        ...
        if holdingNozzle or nozzleInVehicle or nozzleDropped then
            ...
            if nozzle and pumpCoords then
                ...
                if nozzleDropped and #(nozzleLocation - pedCoords) < 1.5 then
                    DrawText3D(nozzleLocation.x, nozzleLocation.y, nozzleLocation.z, "[E] Pickup Charger")
                    if IsControlJustPressed(0, 51) then
            ...
            if veh then
                ...
                elseif vehClass ~= 13 and not config.electricVehicles[GetHashKey(veh)] then
                    ...
                    if tankBone == -1 then
                        tankBone = GetEntityBoneIndexByName(veh, "seat_dside_r")
                        nozzleModifiedPosition.x = -0.4
                        nozzleModifiedPosition.y = -0.5
                        nozzleModifiedPosition.z = -0.3
                        textModifiedPosition.x = -0.4
                        textModifiedPosition.y = -0.5
                        textModifiedPosition.z = -0.3
                ...
                if tankPosition and #(pedCoords - tankPosition) < 3.0 then
                    if not nozzleInVehicle and holdingNozzle then
                        nearTank = true
                        DrawText3D(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "[E] Attach Nozzle")
                        if IsControlJustPressed(0, 51) then
                            LoadAnimDict("timetable@gardener@filling_can")
                            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                            Wait(300)
                    ...
                    elseif nozzleInVehicle then
                        DrawText3D(tankPosition.x + textModifiedPosition.x, tankPosition.y + textModifiedPosition.y, tankPosition.z + zPos + textModifiedPosition.z, "[E] Detach Nozzle")
                        if IsControlJustPressed(0, 51) then
                            LoadAnimDict("timetable@gardener@filling_can")
                            TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
                            Wait(300)
