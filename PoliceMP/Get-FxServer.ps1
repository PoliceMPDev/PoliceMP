...
function Get-FxVersion
{
    ...
    try{
        ...
        if(Test-Path -Path "$PSScriptRoot\server\server")
        {
            Remove-Item -Path "$PSScriptRoot\server\server" -Recurse -ErrorAction Stop
        }

        Expand-7Zip $filePath -DestinationPath "$PSScriptRoot\server\server"
    }
    finally{
