...
namespace PoliceMP.Main.Client.Menus
{
    public class ExampleMenu // : BaseScript
    {
        public ExampleMenu()
        {
            ...
            var menu = new Menu("Main Menu", "Subtitle") { Visible = true };
            ...
            menu.AddMenuItem(new MenuItem("Normal Button",
            ...
            var box = new MenuCheckboxItem("Checkbox - Style 1 (click me!)",
                "This checkbox can toggle the menu position! Try it out.", !menu.LeftAligned)
            {
                Style = MenuCheckboxItem.CheckboxStyle.Tick
            };

            var box2 = new MenuCheckboxItem("Checkbox - Style 2", "This checkbox does nothing right now.", true)
            {
                Style = MenuCheckboxItem.CheckboxStyle.Cross
            };

            var box3 = new MenuCheckboxItem("Checkbox (unchecked + locked)",
                "Make this menu right aligned. If you set this to false, then the menu will move to the left.", false)
            {
                Style = MenuCheckboxItem.CheckboxStyle.Cross,
                Enabled = false,
            ...
            var dynList = new MenuDynamicListItem("Dynamic list item.", "0", ChangeCallback,
            ...
            for (var i = 0; i < 64; i++) colorList.Add($"Color #{i}");
            var hairColors = new MenuListItem("Hair Color", colorList, 0, "Hair color pallete.")
            {
                ShowColorPanel = true
            };
            ...
            for (var i = 0; i < 64; i++) makeupColorList.Add($"Color #{i}");
            var makeupColors = new MenuListItem("Makeup Color", makeupColorList, 0, "Makeup color pallete.")
            {
                ShowColorPanel = true,
            ...
            for (var i = 0; i < 11; i++) opacityList.Add($"Opacity {i * 10}%");
            var opacity = new MenuListItem("Opacity Panel", opacityList, 0, "Set an opacity for something.")
            {
                ShowOpacityPanel = true
            };

            // Normal
            var normalList = new List<string> { "Item #1", "Item #2", "Item #3" };
            var normalListItem = new MenuListItem("Normal List Item", normalList, 0,
            ...
            var submenu = new Menu("Submenu", "Secondary Menu");
            MenuController.AddSubmenu(menu, submenu);

            var menuButton = new MenuItem("Submenu",
            ...
            for (var i = 0; i < 30; i++)
            {
                var tmpItem = new MenuItem($"Icon Sprite #{i}",
                    "This menu item has a left and right sprite, and some also have a right label! Very cool huh?!");
                if (i % 4 == 0) tmpItem.Label = "Wowzers";
                if (i % 7 == 0) tmpItem.Label = "Snailsome!";
            ...
            var menu3 = new Menu(null, "Only a subtitle, no banner.");
            ...
            var thirdSubmenuBtn =
                new MenuItem("Another submenu", "This is just a submenu without a banner. No big deal.")
                { Label = "→→→" };
            ...
            */
            ...
            menu.OnSliderPositionChange += (_menu, _sliderItem, _oldPosition, _newPosition, _itemIndex) =>
            {
                ...
                Debug.WriteLine(
                    $"OnSliderPositionChange: [{_menu}, {_sliderItem}, {_oldPosition}, {_newPosition}, {_itemIndex}]");
            };
            ...
            menu.OnDynamicListItemCurrentItemChange += (_menu, _dynamicListItem, _oldCurrentItem, _newCurrentItem) =>
            {
                ...
                Debug.WriteLine(
                    $"OnDynamicListItemCurrentItemChange: [{_menu}, {_dynamicListItem}, {_oldCurrentItem}, {_newCurrentItem}]");
            };
