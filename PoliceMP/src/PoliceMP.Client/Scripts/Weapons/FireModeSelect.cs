...
namespace PoliceMP.Client.Scripts.Weapons
{
    ...
    public class FireModeSelect : Script
    {
        ...
        private bool _safetyToggleRequested = false;
        ...
        {
            {WeaponHash.Pistol, new List<FireMode> {FireMode.Safety, FireMode.Single}},
            {WeaponHash.CombatPistol, new List<FireMode> {FireMode.Safety, FireMode.Single}},
            {WeaponHash.SNSPistol, new List<FireMode> {FireMode.Safety, FireMode.Single}}, 
            {WeaponHash.SawnOffShotgun, new List<FireMode> {FireMode.Safety, FireMode.Single}}, // Baton Gun
            {WeaponHash.CarbineRifle, new List<FireMode> {FireMode.Safety, FireMode.Single}},
            {WeaponHash.SpecialCarbine, new List<FireMode> {FireMode.Safety, FireMode.Single}},
            {WeaponHash.SniperRifle, new List<FireMode> {FireMode.Safety, FireMode.Single}},
            {WeaponHash.SMG, new List<FireMode> {FireMode.Safety, FireMode.Single, FireMode.Burst}},
            {WeaponHash.AssaultSMG, new List<FireMode> {FireMode.Safety, FireMode.Single, FireMode.Burst}},
            {WeaponHash.SMGMk2, new List<FireMode> {FireMode.Safety, FireMode.Single, FireMode.Burst}},
            {(WeaponHash)API.GetHashKey("WEAPON_SIG716"), new List<FireMode> {FireMode.Safety, FireMode.Single}},
            {(WeaponHash)API.GetHashKey("WEAPON_SIGMCX"), new List<FireMode> {FireMode.Safety, FireMode.Single}},
            {(WeaponHash)API.GetHashKey("WEAPON_TRAININGMP5SEMI"), new List<FireMode> {FireMode.Safety, FireMode.Single, FireMode.Burst}},
        };
        
        #endregion
        
        public FireModeSelect(ITickManager ticks, INotificationService notifications)
        {
            ticks.On(FireModeOnTick);
            ticks.On(ShowCurrentMode);
        }
        ...
        private async Task FireModeOnTick()
        {
            ...
            if (_fireMode == FireMode.Safety)
            {
                ...
                {
                    _notifications.Info("Firearm Safety", "Weapon safety mode is enabled!");
            ...
            if (_safetyToggleRequested)
            {
                _safetyToggleRequested = false;
        ...
        {
            API.RegisterCommand("toggleSafety", new Action<int, List<object>, string>((source, args, raw) =>
            {
                _safetyToggleRequested = true;
            }), false);

            API.RegisterKeyMapping("toggleSafety", "Toggle Weapon Safety", "keyboard", "K");
