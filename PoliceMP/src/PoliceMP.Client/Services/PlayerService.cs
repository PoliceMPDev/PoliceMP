...
namespace PoliceMP.Client.Services
{
    public class PlayerService : Script, IPlayerService
    {
        ...
        public PlayerService(ILegacyClientCommunicationsManager comms, ILogger<PlayerService> logger, ITickManager tickManager)
        {
            _comms = comms;
            _logger = logger;
        ...
        public PlayerInfo FetchCahcedPlayerInfoFromNetworkId(int networkId)
        {
            foreach(var player in _playerInfos)
            {
                if (player.NetworkId != networkId) continue;
