...
namespace PoliceMP.Server.Controllers.AiCallouts.Callouts.NHS

//@todo Need to make it so all the peds can be asked questions and sort questions
{
    public class MentalHealthCrisisCallout : BaseCallout, IAiCallout
    {
        //***
        ...
        private readonly Vector3 patientlocation = new(308.22f, -591.53f, 42.28f);
        private readonly Vector3 doctorlocation = new(309.69f, -594.07f, 43.28f);
        private readonly Vector3 securityguardlocation = new Vector3(305.82f, -593.71f, 43.28f);


        private Vector3 _calloutLocation = new(308.56f, -591.95f, 43.28f);

        public MentalHealthCrisisCallout(IBehaviorService behaviors, AiCalloutsPeds peds, IPedInfoService pedInfo, IXPService xpService)
        {
        ...
        {
            return "Assault on Emergency Worker";
        }
        ...
        {
            return "Hospital is reporting a member of staff has been assaulted by a patient.";
        }
        ...
        public override void OnCreate()
        {
            ...
            doctor = API.CreatePed(
                0,
                (uint)PedHash.Scientist01SMM,
                doctorlocation.X,
                doctorlocation.Y,
                doctorlocation.Z,
                0,
            ...
            securityguard = API.CreatePed(
                0,
                (uint)PedHash.Security01SMM,
                securityguardlocation.X,
                securityguardlocation.Y,
                securityguardlocation.Z,
                0,
                true,
                false);
        }
        ...
        public override async void OnStart()
        {
            ...
            Ped doctorped = Entity.FromHandle(doctor) as Ped;
            Ped securityguardped = Entity.FromHandle(securityguard) as Ped;
            ...
            if (patientpedbehaviour != null)
            {
                ...
                patientpedbehaviour.Set(bb => bb.SpeechTicksInterval, 100);
                patientpedbehaviour.Set(bb => bb.AmbientSounds, ambientSoundList);
            }


            if (null == doctorped) return;
            var doctorpedbehaviour = await _behaviors.SetPedBehavior<PlayAnimationBehavior>(doctorped);
            if (doctorpedbehaviour != null)
            {
                var ambientSoundList = new List<string>()
                {
                    "APOLOGY_NO_TROUBLE"
                ...
                var animationsList = new List<Tuple<string, string>>()
                {
                    new Tuple<string, string>("amb@world_human_bum_standing@twitchy@idle_a", "idle_c")
                };

                doctorpedbehaviour.Set(bb => bb.AnimationsTupleList, animationsList);
                doctorpedbehaviour.Set(bb => bb.Speeches,
                    new List<string>()
                    {
                        "We are trying to section you under 136.", "Please take her away now officers",
                        "Can we request a JRU unit please."
                    });
                doctorpedbehaviour.Set(bb => bb.SpeechTicksInterval, 300);
                doctorpedbehaviour.Set(bb => bb.AmbientSounds, ambientSoundList);
            }


            var securityguardbehaviour = await _behaviors.SetPedBehavior<PlayAnimationBehavior>(securityguardped);
            if (null == securityguardbehaviour) return;
            if (securityguardbehaviour != null)
            {
                var ambientSoundList = new List<string>()
                {
                ...
                var animationsList = new List<Tuple<string, string>>()
                {
                    new Tuple<string, string>("anim@amb@nightclub@peds@", "rcmme_amanda1_stand_loop_cop")
                };

                securityguardbehaviour.Set(bb => bb.AnimationsTupleList, animationsList);
                securityguardbehaviour.Set(bb => bb.Speeches,
                    new List<string>() { "Madam please calm down", "The police have been called." });
                securityguardbehaviour.Set(bb => bb.SpeechTicksInterval, 300);
                securityguardbehaviour.Set(bb => bb.AmbientSounds, ambientSoundList);
            }
        ...
        public override void OnComplete()
        {
            ...
            DeletePedById(doctor);
            DeletePedById(securityguard);
        }
