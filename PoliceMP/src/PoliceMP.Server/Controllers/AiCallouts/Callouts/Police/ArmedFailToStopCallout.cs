...
{
    public class ArmedFailToStopCallout : BaseCallout, IAiCallout
    {
        ...
        public ArmedFailToStopCallout(IBehaviorService behaviors, IPedInfoService pedInfo, IXPService xpService)
        {
        ...
        public override string Title()
        {
            ...
            {
                "Armed Vehicle stolen with keys",
                "Armed Vehicle stolen without keys",
                "Armed Vehicle stolen from burglary",
                "Reports of vehicle on cloned plates",
                "Registered keeper with outstanding firearms warrant",
                "Vehicle wanted in relation to firearms offences"
            };
        ...
        public override async void OnStart()
        {
            ...
            API.GiveWeaponToPed(ped.Handle, AiCalloutsWeapons.ArmedRobberyWeapons.GetRandom(), 4000, false, true);
            API.TaskCombatPed(ped.Handle, int.Parse(_attendees.GetRandom().player.Handle), 0, 16);
            API.TaskShootAtEntity(ped.Handle, int.Parse(_attendees.GetRandom().player.Handle), int.MaxValue, 3337513804);
        ...
        public override IAiCallout OnResolve()
        {
            ...
            if (IsPedDead(_pedId))
            {
                return new CIDSuddenDeathCallout(_pedId, _xpService);
            }
